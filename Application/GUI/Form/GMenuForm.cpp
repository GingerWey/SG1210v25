...
{
  CSG_DrawPicture(CSG_WINATLAS, MU_HOME_X, MU_HOME_Y, MU_HOME_PICIDX);
}
...
static void _DrawMenuItem(int idx, bool bSelect)
{
  ...
  // --- 绘制图标 ---
  CSG_DrawPicture(CSG_WINATLAS, pItem->xIcon, pItem->yIcon, pItem->picIdx);
...
{
  const TMenuItem* pItem = &kMenuItems[m_State.iCurItem];
  gfc::PushForm(pItem->uTargetWID, pItem->pArgument);
}
