...
/*
 ...
 Date        : 2026.07.14 (V1.14 — Yoda-style for variable comparisons by scope)
              ...
              2026.07.14 (V1.12 — complete Yoda-style rectification for touch handler)
              2026.07.14 (V1.11 — use RAM_Malloc/RAM_Free uniformly)
              2026.07.14 (V1.10 — Yoda-style comparisons in conditionals)
              2026.07.14 (V1.09 — 使用 NUM_Elements 宏计算数组元素个数)
...
static void _DrawRegValue(uint32_t uRegNum, int y, bool bSelected, bool bClear)
{
  ...
  if (REG_REAL == REG_TYPE(uRegNum)) {
    ...
    GUI_DrawPicture(CSG_WINATLAS, ix, iy, imgIdx, 100);
  }
}
...
static void _DrawRow(uint16_t uIdx)
{
  ...
  if (kGroup == pRow->kind) {
      if (0 != pRow->regNum) {
          ...
          GUI_DrawPicture(CSG_WINATLAS, ix, iy, pRow->regNum, 100);
      }
...
static void _OnKey(uint16_t uwKey, bool bRepeat)
{
  ...
  case KEY_DOWN: {
    ...
    if (nxt != m_State.uCurItem) {
      ...
      m_State.uCurItem = nxt;
      _EnsureCursorVisible();
    } else if (DL_VISIBLE < s_rowCount &&
               s_rowCount - DL_VISIBLE > m_State.uTopItem) {
...
static void _OnTouch(uint16_t action, int16_t x, int16_t y)
{
  ...
  case TOUCH_DOWN:
    ...
    if (DL_CONTENT_X0 <= x && DL_CONTENT_X1 >= x &&
        DL_CONTENT_Y0 <= y &&
        DL_CONTENT_Y0 + DL_VISIBLE * DL_ROW_H > y) {
  ...
  case TOUCH_MOVE:
    if (true == m_State.touchActive) {
      ...
      if (DL_ROW_H <= dy) {
        ...
        uint16_t shift = (uint16_t)((-dy) / DL_ROW_H);
        if (DL_VISIBLE < s_rowCount) {
          uint16_t maxTop = s_rowCount - DL_VISIBLE;
          uint16_t newTop = m_State.uTopItem + shift;
          m_State.uTopItem = (maxTop < newTop) ? maxTop : newTop;
        }
        m_State.touchLastY = y;
        _UpdateList();
        _DrawScrollbar();
...
static void _Init(const void* argument)
{
  ...
  // Allocate the per-row cache; freed in _Close.
  if (nullptr != s_pCache) {
    RAM_Free(s_pCache);
    s_pCache = nullptr;
  }
