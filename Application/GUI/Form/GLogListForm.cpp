...
/*
 ...
 Date        : 2026.07.14 (V1.06 — Yoda-style for variable comparisons by scope)
              2026.07.14 (V1.05 — use RAM_Malloc/RAM_Free uniformly)
              2026.07.14 (V1.04 — Yoda-style comparisons in conditionals)
              2026.07.13 (V1.03 — Caption left edge: add 4px gap from frame border
...
static void _DrawCaption(void)
{
  ...
  GUI_DrawPicture(CSG_WINATLAS, iconX0, LL_CAP_ICON_Y0,
      uIconId, 100);
  ...
  GUI_DrawPicture(CSG_WINATLAS, LL_CAP_ICON_X0, LL_CAP_ICON_Y0,
...
static void _OnKey(uint16_t uwKey, bool bRepeat)
{
  ...
  case KEY_DOWN: {
    ...
    if (s_pState->uCurItem + 1 < s_pState->uCount) {
      ++s_pState->uCurItem;
      _EnsureCursorVisible();
    } else if (LL_VISIBLE < s_pState->uCount &&
               s_pState->uCount - LL_VISIBLE > s_pState->uTopItem) {
      ++s_pState->uTopItem;
    } else {
      break;
    }
