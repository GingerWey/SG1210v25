...
//=============================================================================
// Enum option lists (spec 6.6) - uint16 arrays of string NameIds, resolved
// to text via GetMultiLangString. The register value indexes the list.
static const uint16_t cbxListBool[]     = { idBoolFalse, idBoolTrue };
static const uint16_t cbxListVolt[]     = { idVolt220, idVolt380 };
static const uint16_t cbxListBaudRate[] = { idUartBaud01, idUartBaud02,
                                            idUartBaud03, idUartBaud04 };
static const uint16_t cbxListParity[]   = { idUartParity0, idUartParity1,
                                            idUartParity2 };
...
static const CfgRow s_rowsLogic[] = {
  ENUM_ROW(REG_FN_AUTOCTRL_EN,     cbxListBool),
...
static const CfgRow s_rowsSerial[] = {
  ...
  ENUM_ROW(REG_UART1_BAUDRATE, cbxListBaudRate),
  ENUM_ROW(REG_UART1_PARITY,   cbxListParity),
};
...
static void _DrawCaption(void)
{
  ...
  GUI_DrawPicture(&picMAUAtlascsg, CF_CAT_ICON_X0, CF_CAT_ICON_Y0,
...
static void _OnKey(uint16_t uwKey, bool bRepeat)
{
  ...
  case KEY_DOWN: {
    ...
    if (s_pState->uCurItem + 1 < s_pState->uCount) {
      ++s_pState->uCurItem;
      _EnsureCursorVisible();
    } else if (s_pState->uCount > CF_VISIBLE &&
               s_pState->uTopItem < s_pState->uCount - CF_VISIBLE) {
      ++s_pState->uTopItem;
    } else {
      break;
    }
