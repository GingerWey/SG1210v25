...
**Additions**:
15. Added GRect structure definition (G.1.1a)
...
```cpp
   GDialogType  dlgType;        // 枚举类型，识别当前正在进行哪类交互
   GDialogTodo  dlgTodo;        // 枚举类型，当前对话框结束后的下一步任务
...
```
**RINF_getRegEnumList已在DevRegInfo.h中实现**
---
...
This appendix provides a structured implementation plan for the dialog system
described in this specification. It covers class hierarchy, implementation order,
dependencies, and a comprehensive checklist for tracking development progress.

### G.2 Architecture Summary

**Class Hierarchy:**
```
GWidget (base, virtual destructor)
├── GDialog (modal dialogs, manages keyboard + editor panel)
│   ├── GNumRegDialog (INT/HEX/REAL registers)
│   ├── GLoginDialog (authorization + password editing)
│   ├── GIPAddressDialog (IPv4 address editing)
│   └── GDatetimeDialog (date/time editing)
├── GListbox (in-place ENUM editor)
├── GKey (single key widget)
├── GKeyboard (key grid, dispatches GKey presses)
├── GEditor (single editor widget, abstract)
│   └── GNumberEditor (numeric input with range/format)
└── GEditorPanel (editor array manager)
```

**Memory Management:**
- All widgets use RAMHeap: `RAM_Malloc` → placement `new` → explicit `~GWidget()` → `RAM_Free`
- Two-phase init: constructor (minimal) + `Init()` (create sub-widgets)
- Uniform teardown: `pWidget->~GWidget()` (virtual dispatch to derived destructor chain)

**Message Flow:**
- Form owns dialog: `s_pState->pDialog` (GWidget*)
- Form intercepts result messages (GM_DIALOG_ACCEPT/CANCEL/YES/NO/OK), forwards others
- Dialog routes physical keys to keyboard (KEY_ENTER/ESCAPE/ARROW) and editors (digits/backspace)
- Keyboard translates key grid to semantic `onGKeyPress(keyCode)` (krDigit0-9, krBackspace, krMovePrev/Next)

### G.3 Implementation Phases

#### Phase 1: Foundation (Base Classes)
**Goal:** Establish widget infrastructure and memory management patterns.

1. **GWidget** (base class)
   - Virtual destructor `~GWidget()`
   - Message handlers: `onMessage()`, `onKeyDown()`, `onShow()`, `onClose()`
   - Touch handlers (if GUI_SUPPORT_TOUCH): `onTouchDown()`, `onTouchMove()`, `onTouchUp()`
   - Geometry: `setRect()`, `getRect()`
   - Parent pointer: `m_pParent`

2. **GEditor** (abstract editor widget)
   - Focus management: `setFocus()`, `getFocus()`
   - Value access: `getValue()`, `setValue()`
   - Validation: `validate()` (range check)
   - Semantic key handler: `onGKeyPress(keyCode, repeat)` (pure virtual)

3. **GNumberEditor** (concrete numeric editor)
   - Range: `setRange(min, max)`
   - Format: `setFormat(pattern)` (e.g., "0000", "000.0")
   - Step: `setStep(step)`
   - Cursor and digit editing logic
   - Implements `onGKeyPress()` for digit input

4. **GEditorPanel** (editor array manager)
   - Init from `GEditorPanelDefine` (count, editor configs)
   - Focus switching: UP/DOWN or krMovePrev/krMoveNext
   - Validation: iterate all editors' `validate()`
   - Array access: `operator[](index)`

**Dependencies:** DevDebug (DEV_ASSERT), RAMHeap (RAM_Malloc/Free), emWin (GUI_FONT, drawing)

**Testing:** Create standalone GNumberEditor, verify digit input, range validation, focus visual.

---

#### Phase 2: Keyboard System
**Goal:** Implement virtual keyboard for touch-based digit/symbol input.

1. **GKey** (single key widget)
   - Config: `GKey::GConfig` (keyCode, label text/icon, position)
   - Visual states: normal, pressed, invalid (uInvalid flag)
   - Focus: `setFocus()` (highlight for arrow-key navigation)
   - Returns: `keyCode` (krDigit0-9, krBackspace, krOK, krCancel, krMovePrev/Next)

2. **GKeyboard** (key grid manager)
   - Init from `GKeyboard::GDefine` (rows, cols, key array)
   - Physical key routing: `onKeyDown(KEY_UP/DOWN/LEFT/RIGHT/ENTER/ESCAPE)`
   - Arrow navigation: skip invalid keys, wrap at edges, call `setFocus()` on target key
   - KEY_ENTER: send focused key's `keyCode` via `pDialog->onGKeyPress(keyCode, repeat)`
   - Touch: hit-test grid, send key's `keyCode` directly

3. **Keyboard Styles**
   - DigitalKeyboard: 0-9, backspace, +/-, OK/Cancel (for numeric input)
   - HexKeyboard: 0-9, A-F, backspace, OK/Cancel (for hex input)
   - Define as `const GKeyboard::GDefine` arrays in config files

**Dependencies:** GWidget, GKey, GDialog (parent for onGKeyPress callback)

**Testing:** Create standalone DigitalKeyboard, verify arrow navigation, key press dispatch, touch hit-test.

---

#### Phase 3: Modal Dialog Base (GDialog)
**Goal:** Implement modal dialog container with keyboard + editor panel lifecycle.

1. **GDialogConfig** structure
   - Background (x, y, w, h, colors, border radius)
   - Keyboard: pKbdStyle, pKeyGrid, kbdX, kbdY, kbdW, kbdH
   - EditorPanel: pEditorStyle, pEditorPanel
   - Labels: pLabelConfigs (array of GLabel::GConfig for static text/icons)

2. **GDialog** class
   - Constructor: save config + param, do NOT create sub-widgets yet
   - `Init()`: create keyboard (RAM_Malloc + placement new + Init), create editor panel, create labels
   - `onShow()`: draw background, show labels, show keyboard, show editors
   - `onClose()`: cleanup before teardown
   - `onKeyDown()`: KEY_ESCAPE→`cancel()`, else forward to keyboard
   - `accept()`: validate editors (if panel exists), post GM_DIALOG_ACCEPT
   - `cancel()`: post GM_DIALOG_CANCEL
   - Virtual `getTitle()`: for derived classes to provide dialog-specific title

3. **Teardown**
   - Destroy keyboard: `m_pKeyboard->~GWidget()` + RAM_Free
   - Destroy editor panel: `m_pEditors->~GWidget()` + RAM_Free
   - Destroy labels: iterate array, `~GWidget()` + RAM_Free
   - Form calls: `pDialog->onClose()` → `pDialog->~GWidget()` → `RAM_Free(pDialog)`

**Dependencies:** GWidget, GKeyboard, GEditorPanel, GLabel (static text/icon widget)

**Testing:** Create minimal GDialog with one editor + DigitalKeyboard, verify Init→show→accept→teardown lifecycle.

---

#### Phase 4: Concrete Dialogs
**Goal:** Implement all modal dialog types for register editing.

1. **GNumRegDialog** (INT/HEX/REAL)
   - Constructor: extract `regNum` from Param, query `DevIntf_GetRegInfo(regNum)`
   - `getTitle()`: return register name from property
   - EditorPanel: single GNumberEditor, range from Min/Max, format from Scale/Width/Decimal
   - Keyboard: DigitalKeyboard (INT/REAL) or HexKeyboard (HEX)
   - `getResult()`: return editor's `getValue()`

2. **GLoginDialog** (authorization + password)
   - Constructor: extract `mode` from Param (lmLoginNormal/lmLoginAdmin/lmPassword/lmDynaPassword)
   - `Init()`: set `m_uRegNum` based on mode (REG_PASSWORD or REG_FN_PASSWORD)
   - `getTitle()`: return title based on mode (idLoginCap1/idLoginCap2/idSetPSWDCap1)
   - EditorPanel: single GNumberEditor (4-digit password, format "0000")
   - `accept()`: validate, then `DevReg_Write(m_uRegNum, value)` (writes password to verify/change)
   - NO `getResult()` — Form checks `GetPasswordOk()` after GM_DIALOG_ACCEPT

3. **GIPAddressDialog** (IPv4)
   - Constructor: extract `regNum` from Param
   - EditorPanel: 4 GNumberEditors (range 0-255, format "000"), separated by "." labels
   - Keyboard: DigitalKeyboard
   - `getResult()`: pack 4 octets into uint32_t `(oct0<<24)|(oct1<<16)|(oct2<<8)|oct3`

4. **GDatetimeDialog** (date/time)
   - Constructor: no specific register (date/time is system-level, not register-based in current spec)
   - EditorPanel: 6 GNumberEditors (YYYY, MM, DD, HH, MM, SS), with range/format per field
   - Keyboard: DigitalKeyboard
   - `getResult()`: return TDateTimeType struct (or packed format per system API)

**Dependencies:** DevIntf (DevReg_Read/Write, DevIntf_GetRegInfo), multi-language strings (GetMultiLangString)

**Testing:** For each dialog type:
- Create from Form, verify Init builds correct editor count
- Input valid/invalid values, verify validation
- Accept, verify `getResult()` returns correct format
- Cancel, verify no side effects

---

#### Phase 5: In-Place Editors
**Goal:** Implement lightweight in-place editors for SIT_VAT_ENUM and SIT_VAT_BIN.

1. **GListbox** (SIT_VAT_ENUM)
   - Constructor: `GListbox(pStyle, pConfig, curIndex)`
   - Config: `GConfig` with ItemCount, pStrIds (or pStrings), position (x,y,w,h), margin, alignment
   - No two-phase Init (single widget, no sub-components)
   - `onShow()`: compute visible rows, draw items, highlight current
   - `onKeyDown()`: UP/DOWN scroll, ENTER→`accept()`, ESCAPE→`cancel()`
   - `onTouchDown()`: click inside→select item→accept, click outside→cancel
   - `getValue()` / `getResult()`: return `m_uCurItem` (selected index)
   - `accept()`: post GM_DIALOG_ACCEPT with `m_uCurItem`
   - `cancel()`: post GM_DIALOG_CANCEL

2. **Direct Switch** (SIT_VAT_BIN)
   - NO widget object created
   - Form handles in `onKeyDown(KEY_ENTER)` or `onTouchDown(data_area)`:
     - Check: `GetPasswordOk() && !SIT_RDOnly && SIT_GetVType==SIT_VAT_BIN`
     - Read: `DevReg_Read(regNum, &value)`
     - Toggle: `value = (STATE_TRUE == value) ? STATE_FALSE : STATE_TRUE`
     - Write: `DevReg_Write(regNum, value)`
     - Refresh: update current row display

3. **Form Integration**
   - `createListBox(regNum)`:
     - Call `getRegEnumList(regNum, &pStrIds)` (see Appendix C.4)
     - Allocate GListbox + GConfig via RAM_Malloc
     - Fill GConfig (position computed from current row geometry, pStrIds, ItemCount)
     - Read current register value as `curIndex`
     - Placement-new: `new (pListbox) GListbox(pStyle, pConfig, curIndex)`
     - Store in `s_pState->pDialog` (GWidget*)
     - Show directly (no Init needed)
   - `acceptEdit()` for GListbox:
     - Cast: `GListbox* pList = (GListbox*)s_pState->pDialog`
     - Get result: `uint32_t index = pList->getResult()`
     - Write: `DevReg_Write(regNum, index)`
     - Refresh row display
   - Teardown: same as modal dialogs (`~GWidget()` + RAM_Free)

**Dependencies:** Appendix C.4 `getRegEnumList()`, register property query (SIT_GetVType, SIT_RDOnly)

**Testing:**
- GListbox: create with 5 items, navigate with UP/DOWN, select with ENTER, touch outside to cancel
- Direct switch: toggle BIN register, verify DevReg_Write called, display updates

---

#### Phase 6: Form Integration
**Goal:** Wire dialogs into GxxxForm lifecycle (permission gate, type dispatch, message handling).

1. **TFormState Extensions**
   - Add fields: `dlgType`, `dlgTodo`, `pDialog`, `pendingRegNum`, `pDlgData`
   - Init: `ClrPasswordOk()` on Form create
   - Close: `ClrPasswordOk()` on Form close

2. **Permission Gate**
   - Before edit: check `GetPasswordOk()`
   - If false: save `pendingRegNum`, set `dlgTodo=dtdEdit`, create GLoginDialog (mode per user level)
   - On GLoginDialog GM_DIALOG_ACCEPT: check `GetPasswordOk() && dlgTodo==dtdEdit`, dispatch edit for `pendingRegNum`

3. **Type Dispatch**
   - Read register property: `pProp = DevIntf_GetRegInfo(regNum)`
   - Switch on `SIT_GetVType(pProp->Property)`:
     - SIT_VAT_INT/HEX/REAL → `createNumberDialog(regNum)`
     - SIT_VAT_BIN → direct toggle (no dialog)
     - SIT_VAT_ENUM → `createListBox(regNum)`
     - SIT_VAT_DATETIME → `createDatetimeDialog()`
     - SIT_VAT_PASSWORD → `createLoginDialog(lmPassword)` (check permission first)
     - SIT_VAT_IPADDRv4 → `createIPAddressDialog(regNum)`

4. **Message Handling**
   - `onMessage()`:
     - If `pDialog != nullptr`:
       - GM_DIALOG_ACCEPT → `acceptEdit()`
       - GM_DIALOG_CANCEL → `cancelEdit()`
       - Other → `pDialog->onMessage(msgId, wParam, lParam)`
     - Else: handle Form's own messages
   - `acceptEdit()`:
     - Call `pDialog->getResult()` (skip for GLoginDialog)
     - Write register: `DevReg_Write(regNum, result)`
     - Destroy dialog: `destroyDialog()`
     - Refresh display
   - `cancelEdit()`:
     - Destroy dialog: `destroyDialog()`
     - Clear `dlgTodo`

5. **Teardown Unification**
   - `destroyDialog()`:
     - `pDialog->onClose()`
     - `pDialog->~GWidget()` (virtual dispatch to GDialog or GListbox destructor chain)
     - `RAM_Free(pDialog)`
     - `pDialog = nullptr`
     - If `pDlgData`: `RAM_Free(pDlgData)`, `pDlgData = nullptr`

**Dependencies:** DevIntf (register read/write, property query), permission API (GetPasswordOk, ClrPasswordOk)

**Testing:**
- Walk through full flow: Form → permission gate → type dispatch → edit → accept → write → refresh
- Test each SIT_VAT_xxx type
- Test permission denial → login → retry edit
- Test cancel at each stage

---

### G.4 Implementation Order

**Recommended sequence** (minimizes rework, enables incremental testing):

1. **Week 1-2: Foundation**
   - GWidget, GEditor, GNumberEditor, GEditorPanel
   - RAMHeap integration, two-phase init pattern
   - Standalone editor test (no dialog, just editor + validation)

2. **Week 3: Keyboard**
   - GKey, GKeyboard
   - DigitalKeyboard / HexKeyboard definitions
   - Standalone keyboard test (arrow nav, key press dispatch)

3. **Week 4: GDialog Base**
   - GDialogConfig, GDialog (Init, show, teardown)
   - Minimal test dialog (1 editor + keyboard)
   - Verify lifecycle: create → Init → show → accept → destroy

4. **Week 5-6: Concrete Dialogs**
   - GNumRegDialog (covers INT/HEX/REAL, most common case)
   - GLoginDialog (special case, no getResult)
   - GIPAddressDialog, GDatetimeDialog (multi-editor panels)
   - Test each in isolation (mock Form message loop)

5. **Week 7: In-Place Editors**
   - GListbox (in-place ENUM)
   - Direct switch (SIT_VAT_BIN, no widget)
   - Appendix C.4 enum option library

6. **Week 8-9: Form Integration**
   - TFormState extensions
   - Permission gate logic
   - Type dispatch switch
   - Message routing (onMessage, acceptEdit, cancelEdit)
   - destroyDialog() unification

7. **Week 10: Integration Testing**
   - End-to-end flows for all register types
   - Permission scenarios (login required, already authorized, timeout)
   - Edge cases (invalid input, cancel mid-edit, rapid open/close)

8. **Week 11: Polish & Optimization**
   - Touch target sizing, visual feedback
   - Performance (minimize redraws, optimize onShow)
   - Memory audit (verify no leaks in create/destroy cycles)

9. **Week 12: Documentation & Handoff**
   - Code comments (match spec section references)
   - Update this spec with any deviations discovered during implementation
   - Developer handoff: architecture walkthrough, test cases

---

### G.5 Dependencies & External APIs

**Required from existing codebase:**

1. **Device Register Interface** (DevIntf.h / DevIntf.cpp)
   - `const TDevRegInfoItem* DevIntf_GetRegInfo(uint32_t regNum)`
   - `bool DevReg_Read(uint32_t regNum, uint32_t* pValue)`
   - `bool DevReg_Write(uint32_t regNum, uint32_t value)`
   - `const char* RINF_GetDIMNameEx(const TDevRegInfoItem* pProp)`

2. **Register Property Macros** (DevRegInfo.h)
   - `SIT_GetVType(prop)` → SIT_VAT_xxx
   - `SIT_GetType(prop)`
   - `SIT_GetDim(prop)`
   - `SIT_GetMulti(prop)`
   - `SIT_SIGNED(prop)`
...
3. **Memory Management** (RAMHeap.h / RAMHeap.cpp)
   - `void* RAM_Malloc(size_t size)`
   - `void RAM_Free(void* ptr)`

4. **Debug Support** (DevDebug.h)
   - `DEV_ASSERT(condition, errorType)` — faults if condition is TRUE

5. **Multi-Language Strings** (TextStrs.h / TextStrs.cpp)
   - `const char* GetMultiLangString(uint16_t strId)`

6. **Permission API** (assumed in DevIntf or separate module)
   - `bool GetPasswordOk()`
   - `void ClrPasswordOk()`

7. **Message Queue** (gfc namespace, GUIMessage.h)
   - `void gfc::PostMsg(uint32_t msgId, uint32_t wParam, uint32_t lParam = 0)`
   - `void gfc::PostMsgPtr(uint32_t msgId, uint32_t wParam, void* ptr)`

8. **emWin Graphics** (GUI.h, emWin SDK)
   - Drawing: `GUI_DrawRect()`, `GUI_FillRoundedRect()`, `GUI_DrawText()`, `GUI_DrawBitmap()`
   - Fonts: `GUI_FONT`, `GUI_SetFont()`
   - Colors: `GUI_COLOR`, `GUI_SetColor()`

**Assumptions:**
- Register numbers (REG_UART1_BAUDRATE, REG_PASSWORD, etc.) are defined in a central header
- Multi-language string IDs (idLoginCap1, idUartParity0, etc.) are defined and localized
- Touch support is conditional on `GUI_SUPPORT_TOUCH` macro
...
1. **Memory Fragmentation**
   - **Risk:** Frequent dialog create/destroy cycles fragment RAMHeap
   - **Mitigation:** Use fixed-size block allocator for common dialog sizes; pool GDialogConfig structs

2. **Touch vs. Keyboard Input Conflicts**
   - **Risk:** Touch on keyboard while arrow-key navigation active → focus desync
   - **Mitigation:** Touch on key clears keyboard focus state before dispatch; test both input paths

3. **GListbox Position Calculation**
   - **Risk:** Listbox overflows screen bounds when row is near top/bottom edge
   - **Mitigation:** Check available space above/below row, anchor listbox accordingly (see §9.2 notes)

4. **Permission Timeout**
   - **Risk:** User authorized, then idle → permission expires mid-edit session
   - **Mitigation:** Spec says "ClrPasswordOk on timeout" — implement timeout timer in Form, call ClrPasswordOk, require re-auth on next edit

5. **Validation Failures**
   - **Risk:** User enters invalid value, presses ENTER → dialog doesn't close, no feedback
   - **Mitigation:** GEditor::validate() returns false → accept() bails early; add visual feedback (flash red border, beep)

6. **Virtual Destructor Dispatch**
   - **Risk:** Calling wrong destructor (e.g., `~GDialog()` on GListbox) → undefined behavior
   - **Mitigation:** Enforce `~GWidget()` in all teardown paths (E-15 fix); code review + static analysis
