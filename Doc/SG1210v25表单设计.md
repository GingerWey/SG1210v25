...
1. 创建：
   ...
   - Init以mltFault为参数进入
2. 退出
   - 按键操作：ESC键切回GMenuForm
   - 触摸操作：单击Menu按键图标，切回GMenuForm
...
2. 刷新策略
   - 日志内容进入表单时加载，不动态刷新
   - 分类切换（LEFT/RIGHT）时重新从 DevEvtMgr 加载新分类的日志
   - 列表滚动（UP/DOWN）只改变显示范围，不重新加载

3. Log类型切换
   - 按键操作：左右键切换配置数据分类
   - 触摸操作：向右或向左滑动，切换配置数据分类
```mermaid
graph LR
A[Touch] --> B1[L->R]
...
表 5-1
| 日志类型 | logType | 图片序号 | Caption文字Id |
|---------|---------|---------|--------------|
| 自动控制 | mltAutoCtrl | picIdxLV_LogA20x20Cyan |idEventCatalog3|
| 装置事件 | mltDevStatus | picIdxLV_LogB20x20Cyan |idEventCatalog2|
| 运行日志 | mltDevLog | picIdxLV_LogC20x20Cyan |idEventCatalog1|

4. Caption
   - 元件：文本框
     - 区域：Caption图标左侧，间距4像素
     - 字体：GUI_FONT_16LTH_CHN
...
ConfigForm是给4个配置菜单项共用：
表6-1
|参数类型| 菜单项 | Init的参数 |
|---|----|----|
|逻辑参数|Logic-Config|cgtLogic|
|设备参数|Device-Config|cgtDevice|
|串行参数|Serial-Config|cgtSerial|
|以太网参数|Ethernet-Config|cgtEthernet|

1. 风格与LogListForm一致，顶部有Caption，用于显示分类名称
2. 行表支持触摸滑动，同时支持按键移动

### 6.1 创建和退出
1. 创建：
   - 从GMenuForm的FatalLog菜项激活后弹出
   - 不同的菜单图标，根据表6-1规定的参数传给Init，决定配置寄存器分类
2. 退出
   - 按键操作：ESC键切回GMenuForm
   - 触摸操作：单击Menu按键图标，切回GMenuForm
...
2. 刷新策略
   - 配置寄存器内容进入表单时加载，不动态刷新
   - 分类切换（LEFT/RIGHT）时重新从 加载配置寄存器分类
   - 列表滚动（UP/DOWN）只改变显示范围，不重新加载

3. 显示模式切换
   - 进入ConfigDialog时，采用【显示】模式，即所有被编辑寄存器的值被显示，但不可以编辑
   - 触发LoginDialog，输入密码；若密码正确，则ConfigForm进入【编辑】模式
     - 按钮操作：
       - 【显示】模式下，在行表任意行上按Enter键，触发登录操作
     - 触摸操作：
       - 点击“Edit按钮”图标，启动LoginDialog，触发登录操作
   - 【显示】模式：
     - 按键操作：在行表任意行上按Enter键，触发登录操作；其它按键操作行表
     - 触摸操作：单击操作无响应，滑动操作可切换行或编辑类型
   - 【编辑】模式
     - 按键操作：在行表任意行上按Enter键，起动编辑器编辑该行寄存器值；其它按键操作行表
     - 触摸操作：单击操作起动编辑器编辑该行寄存器值；滑动操作可切换行或编辑类型
4. 切换配置寄存器分组：
     - 按键操作：左右键切换配置寄存器分类
     - 触摸操作：向右或向左滑动，切换配置寄存器分类
```mermaid
graph LR
A[Touch] --> B1[L->R]
  B1 --> B1A{configType} --> C1[cgtLogic] --> C2[cgtDevice] --> C3[cgtSerial] --> C4[cgtEthernet] --> C1
  B1A --> C2
  B1A --> C3
  B1A --> C4
A --> B2[R->L]
  B2 --> B2A{configType} --> C5[cgtEthernet] --> C6[cgtSerial] --> C7[cgtDevice] --> C8[cgtLogic] --> C5
  B2A --> C6
  B2A --> C7
  B2A --> C8
```
...
表 6-2
| 配置类型 | 图片序号 |Caption文字Id |
|---------|---------|---------|
| 定值参数 | picIdxCF_Logic16x16Cyan  |idCfgGroup01|
| 设备参数 | picIdxCF_Device16x16Cyan |idCfgGroup02|
| 串口参数 | picIdxCF_Serial16x16Cyan |idCfgGroup03|
| 以太网参数 | picIdxCF_Ethernet16x16Cyan |idCfgGroup04|
...
6. Edit按钮
   - 元件：图标
     - 区域：Menu按钮左侧，间距8像素
     - 图集：CSG_WINATLAS
     - 图片：picIdxCF_Edit20x20Cyan
     - 动作：启动 LoginDialog 进行密码校验

#### 6.3.2 Layer1元件

1. 行表
   - 元件：手动ownerdraw列表，用于显示配置数据寄存器项
      - 名称：实时列表
      - 区域：参考LogListForm
      - 显示方式：Ownerdraw
      - 字体：GUI_FONT_16LTH_CHN
...
### 6.4 配置寄存器的类型
1. 在寄存器属性 TDevRegInfoItem.Property 中定义了配置数据寄存器的显示和编辑
2. 配置数据寄存器的值的显示和编辑方式由 SIT_GetVType 宏获得
3. 以下表中的编辑方式、最大值、最小值、步长，均在TDevRegInfoItem中的定义，无须在代码中另行定义
4. Enum备选项是uint16的数组，标记各备选项的NameId，由GetMultiLangString获取
5. 各类寄存器值的显示和编辑方式：

表6-3
|寄存器值类型| 类型说明 | 显示方式 |编辑方式|
|-----------|---------|---------|-------|
| SIT_VAT_INT       | 整数型   | 根据Scale和Width显示|数值编辑器<br>根据Min、Max和Step编辑|
| SIT_VAT_HEX       | 整16进制数据|根据Scale和Width显示|数值编辑器<br>根据Min、Max和Step编辑|
| SIT_VAT_REAL      | 实数型   | 根据Scale、Decimal和Width显示|数值编辑器<br>根据Min、Max和Step编辑|
| SIT_VAT_BIN       | 开关型   | ON = picIdxLV_SwitchON32x16Red<br> OFF= picIdxLV_SwitchOFF32x16Cyan|Combobox编辑器|
| SIT_VAT_ENUM      | 枚举数据 | 从批定的选项列表中取值显示|Combobox编辑器<br>在选项列表中选项编辑|
| SIT_VAT_DATETIME  | 日期时间 | 日期时间字符串| Datatime编辑器<br>专用对话框 | 
| SIT_VAT_PASSWORD  | 密码     | 不显示 | Password编辑器<br>专用对话框 |
| SIT_VAT_IPADDRv4  | IPv4地址 | IPv4地址字符串 | IPv4地址编辑器<br>专用对话框 |

### 6.5 配置寄存器分类

#### 6.5.1 定值及压板寄存器 cgtLogic

表6-4
|寄存器| 编辑方式 | 最小值 | 最大值 | 步长 | Enum备选项 |
|-----|---------|--------|-------|------|------------|
|REG_FN_AUTOCTRL_EN     | Combobox | STATE_FALSE | STATE_TRUE  | | cbxListBool |
|REG_FN_AUTO_TURNOFF    | Combobox | STATE_FALSE | STATE_TRUE  | | cbxListBool |
|REG_FN_AUTO_BREAKER_ON | Combobox | STATE_FALSE | STATE_TRUE  | | cbxListBool |
|REG_FN_PASSBY_EN       | Combobox | STATE_FALSE | STATE_TRUE  | | cbxListBool |
|REG_FN_ACTION_VOLTAGE  | NumberEditor   | 50  | 95  | 1 | |
|REG_FN_COIL_VOLTAGE    | combobox       | 220 | 380 |   | cbxListVolt |
|REG_FN_PWRON_TIME      | NumberEditor   | 1   | 120 | 1 | |
|REG_FN_PWROFF_TIME     | NumberEditor   | 1   | 120 | 1 | |
|REG_FN_SHUTDOWN_TIME   | NumberEditor   | 1   | 120 | 1 | |

#### 6.5.2 设备参数寄存器 cgtDevice

表6-5 
|寄存器| 编辑方式 | 最小值 | 最大值 | 步长 | Enum备选项 |
|-----|---------|--------|-------|------|------------|
|REG_FN_RELAY_DELAY  | NumberEditor   | 10   | 24000  | 10 | |
|REG_FN_RELAY_TIME   | NumberEditor   | 10   | 24000  | 10 | |

#### 6.5.3 串行通信参数寄存器 cgtSerial

表6-6 
|寄存器| 编辑方式 | 最小值 | 最大值 | 步长 | Enum备选项 |
|-----|---------|--------|-------|------|------------|
|REG_UART1_ADDR     | NumberEditor | 1    | 247    | 1 | |
|REG_UART1_BAUDRATE | combobox     | 4800 | 115200 | cbxListBaudRate | |
|REG_UART1_PARITY   | combobox     |      |        | cbxListParity   | |

#### 6.5.4 以太网参数寄存器 cgtEthernet
暂时空白

### 6.6 Enum备选项
Enum备选项是uint16的数组，标记各备选项的NameId，由GetMultiLangString获取

### 6.6.1 cbxListBool
idBoolFalse， idBoolTrue

### 6.6.2 cbxListVolt
idVolt220， idVolt380

### 6.6.3 cbxListBaudRate
idUartBaud01，idUartBaud02，idUartBaud03，idUartBaud04
...
1. 寄存器属性：TDevRegInfoItem
2. 取寄存器属性：DevIntf_GetRegInfo(uRegNum)
3. 寄存器特性：TDevRegInfoItem::Property
4. 寄存器的值和编辑类型：SIT_GetVType(uRegNum)
   - 返回的值，对应表6-3
5. 寄存器的量纲：RINF_GetDIMNameEx

### 6.8 代码生成步骤

1. 由于各类编辑器对话框未设计，当前只完成【显示】模式相关代码。
   - 建立 6.6 Enum备选项 中的各const表
   - 将上述 Enum备选项表 应用到代码中
   -  
