...
| SIT_VAT_BIN       | 开关型   | ON = picIdxLV_SwitchON32x16Red<br> OFF= picIdxLV_SwitchOFF32x16Cyan|直接切换STATE_TRUE/FALSE|
| SIT_VAT_ENUM      | 枚举数据 | 由RegNum查到备选项集，RegNum中的值是索引，显示备选项|GListbox|
| SIT_VAT_DATETIME  | 日期时间 | 日期时间字符串| GDatetimeDialog | 
| SIT_VAT_PASSWORD  | 密码     | 不显示 | GPasswordDialog |
...
#define GM_ISEDITING         136   // In Editting (E-13 CLARIFIED: Reserved for future in-place editor feature)
#define GM_CANCELEDIT        137   // Cancel editting (E-13 CLARIFIED: Reserved for future in-place editor feature)
```
...
A --> B4[==SIT_VAT_BIN]  --> G2["GInplaceDialog<br>(E-14: Future Work)"]
A --> B5[==SIT_VAT_ENUM] --> G2
A --> B6[==SIT_VAT_DATETIME] --> G3[GDatetimeDialog]
A --> B7[==SIT_VAT_PASSWORD] --> G4[GLoingDialog mode=lmPassword]
A --> B8[==SIT_VAT_IPADDRv4] --> G5[GIPAddressDialog]
```

**Note on In-Place Editor** (E-14 CLARIFIED): GInplaceDialog and in-place editing
for SIT_VAT_BIN/ENUM register types is reserved for future implementation. Unlike
modal dialogs that overlay the entire screen, in-place editors would edit values
directly within the data list row. When implemented, they will use the reserved
GM_ISEDITING and GM_CANCELEDIT messages (E-13). See Appendix F for planned design.
...
5. Both paths: Form calls `destroyDialog()` which executes:
   ...
   - `pDialog->~GDialog()` - explicit destructor (virtual, calls derived)
...
public:
  ...
  struct GConfig
  {
    ...
    uint8_t          ItemCount;
    const char*      pStrings;         // string list, 优先级 > pIdItems
...
3. 根据当前行表动态计算GListbox显示区域
   ...
   - 根据待选项项数和可用空间，共同决定行表可见行数
     ...
     - GListbox的宽度 = min(Form行表宽度, 备选项最大宽度 + Define::margin * 2)
4. 备选项
...
void GxxxForm::createListBox(uint32_t regNum) 
{
  ...
  memset(pConfig, 0, sizeof(sizeof(GListBox::GConfig)));
  // fill GListBox::GConfig
  ...

  // Current index
  int curIndex = index of Items
  
  s_pState->pDialog = new (ptr) GNumRegDialog(pStyle, pConfig, curIndex);
  if (nullptr != s_pState->pDialog) {
    // save pointer
...
void GxxxForm::destroyListbox() 
{
  if (nullptr != s_pState->pDialog) {
    ...
    // E-15 CLARIFIED: 显式调用析构函数。虽然语法上调用~GDialog()，
    // 但因为是虚析构函数，实际执行完整的派生类析构链：
    // GDialog::~GDialog() → GWidget::~GWidget()
    // 这确保所有派生类资源正确释放。
    s_pState->pDialog->~GDialog();   
    RAM_Free(s_pState->pDialog);
    s_pState->pDialog = nullptr;
...
```mermaid
graph LR
A1[GxxxForm::onKeyDown] --> B1{uKey==KEY_ENTER} --> C1[true] --> D{SIT_GetVType==?} --> E[SIT_VAT_BIN]
A2[GxxxForm::TOUCH_DOWN] --> B2{在行表数据} --> C2[true] --> D
D[value=Reg_Read] --> E{value==?} --> F1[STATE_TRUE] --> G1[value=STATE_FALSE] --> H
E --> F2[STATE_FALSE] --> G2[value=STATE_TRUE] --> H
H[GxxxForm update current list item]
```
### 10.2 应用
...
// ...
if (nullptr != s_pState->pDialog) {
  s_pState->pDialog->~GDialog();   // Explicit destructor
  RAM_Free(s_pState->pDialog);
  s_pState->pDialog = nullptr;
}
```
...
| E-14| P2       | ambiguous   | flow @1183      | In-place-Editor branch is an unspecified stub   | **✅ CLARIFIED v2.21** |
| E-15| P2       | wording     | 3.8.2           | explicit ~GDialog() reads as base-only destroy  | **✅ CLARIFIED v2.21** |
...
## Appendix F: Future Work - In-Place Editor (E-14)

### F.1 Overview

**GInplaceDialog** is reserved for future implementation to support in-place editing
of simple register types (SIT_VAT_BIN, SIT_VAT_ENUM) directly within the data list
row, without overlaying a full-screen modal dialog.
...
Two message IDs are reserved for in-place editor coordination:

```cpp
#define GM_ISEDITING         136   // Sent when in-place editor activates
#define GM_CANCELEDIT        137   // Sent to cancel in-place editing
```
